...
# Backend устанавливается до подготовки custom-профилей: утилитам разметки
# IMGT нужны Biopython и тот же Python 3.12, который используется приложением.
...
RUN set -eu; \
    if [ "$IGBLAST_REQUIRED" = "1" ]; then \
        ...
        test -f /tmp/igblast-source/tools.sha256; \
        ...
        (cd /tmp/igblast-source && sha256sum --check tools.sha256); \
        pip install --no-cache-dir --no-deps /tmp/igblast-source/tools/receptor_utils-0.0.67-py3-none-any.whl; \
        ...
        for specification in \
            ...
            'ov:Ovis_aries' \
            'bv:Bos_taurus'; do \
            profile="${specification%%:*}"; species="${specification#*:}"; \
            source_root="/tmp/imgt/IMGT_V-QUEST_reference_directory/$species/IG"; \
            profile_root="/opt/igblast/profiles/$profile"; \
            database_root="$profile_root/databases"; \
            igdata_root="$profile_root/igdata"; \
            test -f "$profile_root/profile.json"; \
            mkdir -p "$database_root/source" "$igdata_root/internal_data/$profile" "$igdata_root/optional_file"; \
            for segment in IGHV IGHD IGHJ IGKV IGKJ IGLV IGLJ; do \
                test -s "$source_root/$segment.fasta"; \
                cp "$source_root/$segment.fasta" "$database_root/source/$segment.fasta"; \
            done; \
            ...
            for chain in H K L; do \
                python /tmp/igblast-source/prepare_imgt_fasta.py "$database_root/imgt_${profile}_${chain}_v.gapped.fasta" "$source_root/IG${chain}V.fasta"; \
                make_igblast_ndm "$database_root/imgt_${profile}_${chain}_v.gapped.fasta" "V${chain}" "$igdata_root/internal_data/$profile/${profile}_${chain}.ndm.imgt"; \
            done; \
            cat "$igdata_root/internal_data/$profile/${profile}_H.ndm.imgt" "$igdata_root/internal_data/$profile/${profile}_K.ndm.imgt" "$igdata_root/internal_data/$profile/${profile}_L.ndm.imgt" > "$igdata_root/internal_data/$profile/${profile}.ndm.imgt"; \
            annotate_j "$database_root/imgt_${profile}_j.fasta" "$igdata_root/optional_file/${profile}_gl.aux"; \
            test -s "$igdata_root/internal_data/$profile/${profile}.ndm.imgt"; \
            test -s "$igdata_root/optional_file/${profile}_gl.aux"; \
        done; \
        profile='gt'; species='Capra_hircus'; \
        source_root="/tmp/imgt/IMGT_V-QUEST_reference_directory/$species/IG"; \
        profile_root="/opt/igblast/profiles/$profile"; \
        database_root="$profile_root/databases"; \
        igdata_root="$profile_root/igdata"; \
        test -f "$profile_root/profile.json"; \
        mkdir -p "$database_root/source" "$igdata_root/internal_data/$profile" "$igdata_root/optional_file"; \
        for segment in IGKV IGKJ IGLV IGLJ; do \
            test -s "$source_root/$segment.fasta"; \
            cp "$source_root/$segment.fasta" "$database_root/source/$segment.fasta"; \
        done; \
        cat "$source_root/IGKV.fasta" "$source_root/IGLV.fasta" > "$database_root/imgt_${profile}_v.raw.fasta"; \
        cat "$source_root/IGKJ.fasta" "$source_root/IGLJ.fasta" > "$database_root/imgt_${profile}_j.raw.fasta"; \
        for region in v j; do \
            perl /opt/igblast/bin/edit_imgt_file.pl "$database_root/imgt_${profile}_${region}.raw.fasta" > "$database_root/imgt_${profile}_${region}.fasta"; \
            makeblastdb -parse_seqids -dbtype nucl -in "$database_root/imgt_${profile}_${region}.fasta" -out "$database_root/imgt_${profile}_${region}"; \
        done; \
        for chain in K L; do \
        ...
        cat "$igdata_root/internal_data/$profile/${profile}_K.ndm.imgt" "$igdata_root/internal_data/$profile/${profile}_L.ndm.imgt" > "$igdata_root/internal_data/$profile/${profile}.ndm.imgt"; \
        annotate_j "$database_root/imgt_${profile}_j.fasta" "$igdata_root/optional_file/${profile}_gl.aux"; \
        test -s "$igdata_root/internal_data/$profile/${profile}.ndm.imgt"; \
        test -s "$igdata_root/optional_file/${profile}_gl.aux"; \
